...
void DB_TableDelegate::paint(QPainter *painter, const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    case ColumnType::FileNameColumn: {
        QRect pixmapRect = QRect(option.rect.x(), option.rect.y() +
                                 (option.rect.height() - DB_ApplicationScale::getScaleWidth(18))/2,
                                 DB_ApplicationScale::getScaleWidth(16), DB_ApplicationScale::getScaleWidth(18));
        int x = pixmapRect.x() + pixmapRect.width() + DB_ApplicationScale::getScaleWidth(10);
...
#if 1
        int spacing = DB_ApplicationScale::getScaleWidth(BUTTON_SPACING);
...
QSize DB_TableDelegate::sizeHint(const QStyleOptionViewItem &option, const QModelIndex &index) const
{
    ...
    if (-1 != value) {
        return QSize(value, DB_ApplicationScale::getScaleWidth(ITEM_HEIGHT));
    }
