...
Widget_1::Widget_1(QWidget *parent) : QWidget(parent)
{
    m_pRadius = 0;
...
void Widget_1::mSetText(const QString &text)
{
    m_pText = text;
    update();
}

void Widget_1::mSetFont(const QFont &font)
{
    m_pFont = font;
}

void Widget_1::mSetMargin(int left, int right)
{
    m_pLeftMargin = left;
...
void Widget_1::mSetHandleIcon(const QPixmap &pixmap)
{
    m_pHandle->setPixmap(pixmap);
}

void Widget_1::mSetHandleSize(const QSize &size)
{
    m_pHandle->setFixedSize(size);
    m_pHandle->move(QPoint(m_pLeftMargin, (this->height() - m_pHandle->height())/2));
}

void Widget_1::mSetRadius(int radius)
{
    m_pRadius = radius;
}

bool Widget_1::eventFilter(QObject *watched, QEvent *event)
{
...
void Widget_1::paintEvent(QPaintEvent *event)
{
    QWidget::paintEvent(event);
    QPainter painter(this);
    painter.setRenderHint(QPainter::Antialiasing);
    painter.setPen(QPen(QBrush(QColor(224, 224, 224)), Public_Function::getScaleSize(1)));
    ...
    if (!m_pText.isEmpty()) {
        ...
        painter.drawText(QRect(m_pLeftMargin, 0, this->width() - m_pLeftMargin - m_pRightMargin - Public_Function::getScaleSize(16),
