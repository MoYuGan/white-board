    ...
    ui(new Ui::Keyboard)
{
    ...
    this->move((desktop->width() -this->width())/2,desktop->height() -2*this->height());
}
...
void Keyboard::initCustomText(int n)
{
    ...
    QIcon icon(QPixmap(":/images/main/mainpixmap/lock@4x.png").scaled(Public_Function::getScaleSize(32),Public_Function::getScaleSize(32),Qt::IgnoreAspectRatio,Qt::SmoothTransformation));
...
void Keyboard::focusChanged(QWidget *, QWidget *nowWidget)
{
    if (nowWidget != 0 && !this->isAncestorOf(nowWidget))
    {
        if (nowWidget->inherits("QLineEdit"))
        {
            ...
            connect(currentLineEdit,&QLineEdit::cursorPositionChanged,this,[=](int ,int ){
                str.clear();
