...
ExportFileWidget::ExportFileWidget(QList<QPixmap> *pixmapList, const QString path, QWidget *parent) :
    ...
    ui(new Ui::ExportFile)
{
    ...
    m_pWorker = new ExportFileWorker(m_PixmapList, path);
...
void ExportFileWidget::initWidget()
{
    ...
    ui->label_icon->setPixmap(QPixmap(":/images/daochu@2x.png"));

    int pageSize = m_PixmapList->size();
    ui->horizontalSlider->setMinimum(0);
    ui->horizontalSlider->setMaximum(pageSize);
    ui->label_title->setText(tr("保存图片"));
...
void ExportFileWidget::resize()
{
    ...
    ui->frame->setStyleSheet(QString("#frame{"
                                     "border: none;"
                                     "background: white;"
                                     "border-radius: %1px;}").arg(PublicFunction::getScaleSize(12)));
